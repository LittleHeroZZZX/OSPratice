...
size_t* get_blocks(super_block* sb, fcb* fcb)
{
	...
	{
		memcpy(blocks + LEVEL0_BLOCK_CNT, index_to_addr(sb, fcb->mixed_index_block[LEVEL0_BLOCK_CNT]), sizeof(size_t) *
			(block_cnt <
				LEVEL1_BLOCK_CNT +
					LEVEL0_BLOCK_CNT
			 ? block_cnt -
...
void f_read(super_block* sb, user_open* _user_open, void* buf, size_t size)
{
	...
	size_t rest_size = _fcb->length > size ? size : _fcb->length;
	size_t offset = _user_open->p_WR;
	size_t* blocks = get_blocks(sb, _fcb);
	size_t block_cnt = (_fcb->length + BLOCK_SIZE - 1) / BLOCK_SIZE;
...
void f_write(super_block* sb, user_open* _user_open, void* buf, size_t size)
{
	...
	case 0b100:
		...
		concat_blocks(sb, _fcb, new_blocks, new_block_cnt);
		break;
	case 0b000:
		// override mode
		_user_open->p_WR = 0;
...
void save_blocks(super_block* sb, fcb* fcb, size_t* blocks, size_t block_cnt)
{
	...
	if (block_cnt > LEVEL0_BLOCK_CNT)
	{
		...
		memcpy(index_to_addr(sb, fcb->mixed_index_block[LEVEL0_BLOCK_CNT]), blocks + LEVEL0_BLOCK_CNT, sizeof(size_t) *
	...
	if (block_cnt > LEVEL0_BLOCK_CNT + LEVEL1_BLOCK_CNT)
	{
		...
		for (size_t i = 0; i < level1_block_cnt; i++)
			memcpy(index_to_addr(sb, level1_blocks[i]),
				blocks + LEVEL0_BLOCK_CNT + LEVEL1_BLOCK_CNT + i * LEVEL1_BLOCK_CNT, sizeof(size_t) * (block_cnt <
					LEVEL0_BLOCK_CNT + LEVEL1_BLOCK_CNT + (i + 1) * LEVEL1_BLOCK_CNT ? block_cnt - LEVEL0_BLOCK_CNT
																										   - LEVEL1_BLOCK_CNT
																										   - i
																											   * LEVEL1_BLOCK_CNT
																					 : LEVEL1_BLOCK_CNT));
...
void concat_blocks(super_block* sb, fcb* _fcb, size_t* blocks, size_t block_count)
{
	// ToDo
}
